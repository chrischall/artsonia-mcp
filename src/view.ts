 ...
 *
 * The read tools in this server hand back Artsonia's payload close to
 ...
 * honestly say which of Artsonia's fields matter and which are noise.
 *
 ...
 */
export const ART_VIEWS = ['compact', 'full'] as const;

const NOTE =
  'compact strips image/avatar URLs from the response; "full" returns Artsonia\'s payload untouched. ' +
  'No field projection: this server has no verified record of which Artsonia fields matter, and inventing ' +
  'one would risk dropping a field a caller needs.';

/** The `view` parameter every read tool in this server takes. */
export const viewArg = (): ReturnType<typeof viewParam> => viewParam(ART_VIEWS, { note: NOTE });
...
export function viewResponse(view: string | undefined, data: unknown): ReturnType<typeof minifiedResult> {
  const rung: View = resolveView(view, ART_VIEWS);
