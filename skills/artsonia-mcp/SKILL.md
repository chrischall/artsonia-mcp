...
The write tools (`artsonia_post_comment`, `artsonia_invite_fan`, `artsonia_set_notifications`, `artsonia_mark_feedback_read`, `artsonia_download_artwork`) ask the user to confirm first. Where the client can show a confirmation prompt, it does. Otherwise the first call changes nothing and returns `status: "confirmation-required"` with a `preview` of exactly what would be sent or written and a `confirmToken`: show the preview to the user, and only after they approve in chat call the tool again with the **same arguments** plus `confirmToken`. A token works once (`TOKEN_REUSED`), expires, and is refused as `DRAFT_CHANGED` (with a fresh preview and token) if the arguments or the data it acts on changed in between. `MCP_CONFIRM_MODE` (`ask-user` default / `auto` / `refuse`) controls this fallback.
