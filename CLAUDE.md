...
- **Expiry heuristic (`looksUnauthenticated`):** Artsonia expires by redirecting/rendering back to the login page (NOT a 401). Detected from the final URL, a manual 3xx `Location` pointing at `login.asp`, or the login *form structure* rendered in place (`<form action="…login.asp">` plus a `name="Password"` field). Never from body prose — artwork pages carry user-generated text (fan comments, titles), and a comment saying "You need to log in" must not force a re-login or a false bad-credentials error.
...
All tools are `artsonia_*`-prefixed. R = read-only, W = write (confirmation-gated). Reads/writes route through `ArtsoniaClient.fetchHtml()` / `.write()`.
...
- **Confirmation-gating.** Every write (including `download_artwork`, which writes files) goes through `requireConfirmationWithFallback(ctx, confirmationFromEnv({...}))` from `@chrischall/mcp-utils` and takes an optional `confirmToken` (`confirmTokenParam`) — there is no `confirm` parameter. A client that can be prompted gets an elicitation prompt showing the details. One that cannot (claude.ai, Claude Desktop) gets the token fallback, per `MCP_CONFIRM_MODE` (`ask-user` default / `auto` / `refuse`): phase 1 returns `status: "confirmation-required"`, the preview (`wouldSend` / resolved filenames + estimated bytes) and a `confirmToken`, and makes **no mutating network call**; phase 2 repeats the call with the token. The token is HMAC-bound to the tool, target and a hash of the exact payload, single-use (`TOKEN_REUSED`) and time-limited, and the `subject` is rebuilt from the args and a fresh read on every call, so a change between the phases is refused as `DRAFT_CHANGED`. Reads that decide the payload run on every call — `set_notifications` reads `/members/profile/` (the whole re-sent form is bound, so a profile edited in between is refused), `download_artwork` reads the portfolio/detail pages (the artwork list and filenames are bound) — but never a write before the gate. Pre-write guards (e.g. `set_notifications`' "specify at least one") run before the gate, unchanged.
