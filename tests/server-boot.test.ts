...
// End-to-end boot guard. A shipped 0.1.0 crashed on launch because the bundled
// .mcpb eagerly imported the externalized @fetchproxy/server (which isn't present
// in the bundle's runtime) and because the npm `bin` pointed at a path tsc never
// emitted. These tests spawn the REAL built artifacts and confirm they answer
// tools/list — exactly what an MCP host does at install time.
...
function listToolsViaStdio(entry: string, cwd: string): Promise<string[]> {
  return new Promise((resolve, reject) => {
    const child = spawn('node', [entry], {
      ...
      env: { ...process.env, ARTSONIA_USERNAME: '', ARTSONIA_PASSWORD: '' },
    ...
    child.stdout.on('data', (d) => {
      ...
      for (const line of out.split('\n')) {
        ...
        if (msg.id === 1 && msg.result) {
          clearTimeout(timer);
          child.kill('SIGKILL');
          resolve((msg.result.tools ?? []).map((x: any) => x.name));
          return;
        }
      }
    });
...
describe('server boot (built artifacts)', () => {
  it('bundled .mcpb (dist/bundle.js) boots WITHOUT node_modules and lists all tools', async () => {
    // Copy only the bundle into a fresh dir with no node_modules — the .mcpb runtime.
    const dir = mkdtempSync(join(tmpdir(), 'artsonia-mcpb-'));
    try {
