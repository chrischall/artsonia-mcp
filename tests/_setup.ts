...
// with ARTSONIA_USERNAME + ARTSONIA_PASSWORD set would read and write
// ~/.artsonia-mcp/session.json — non-hermetic, order-dependent, and able to
...
const CACHE_DIR = mkdtempSync(join(tmpdir(), 'artsonia-test-cache-'));

beforeEach(() => {
  process.env.ARTSONIA_SESSION_CACHE = 'false';
  process.env.ARTSONIA_SESSION_FILE = join(CACHE_DIR, 'session.json');
});

afterAll(() => {
  rmSync(CACHE_DIR, { recursive: true, force: true });

  const leaked = join(homedir(), '.artsonia-mcp');
  if (existsSync(leaked)) {
    ...
    throw new Error(
      `A test wrote to ${leaked}. The suite must never touch the real home ` +
        'directory — inject ARTSONIA_SESSION_CACHE=false (or a temp ' +
        'ARTSONIA_SESSION_FILE) into the env that test hands the client.',
    );
  }
});
